...
- OAuth 遇到平台限额时优先采用上游给出的重置时间，在恢复前直接使用同名 API 回退；无法取得重置时间时，为当前配置中每个启用的 OAuth 账号建立整点每小时探测，探测成功后自动恢复。恢复探测只使用账号实际公布的模型，不会把手动补填但尚未被平台支持的模型误标为可用。
...
- 保存并应用时会把独立的 `codex-router` provider 合并到 OpenCode 的 `~/.config/opencode/opencode.json`，保留已有 provider，并通过 `CODEX_ROUTER_API_KEY` 环境变量使用本机路由。快捷渠道同时提供官方 OpenCode Go 预设，默认使用 `https://opencode.ai/zen/go/v1` 与 `gpt-5.6-luna`。
...
- Codex 访问本机路由所需的随机 LocalApiKey 会自动生成，以 Windows Credential Manager 为权威存储，同时写入 `CODEX_ROUTER_API_KEY` 供 OpenCode 使用，并作为 `experimental_bearer_token` 写入本机 Codex Router 配置。它只对回环地址上的本机路由有效，不是第三方供应商 Key。
