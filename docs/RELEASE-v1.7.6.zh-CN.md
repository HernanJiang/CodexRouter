...
- 修复全新安装首次 OAuth 登录前“本地 Router 未能稳定启动”：首次引导直接使用当前向导配置启动服务，无需先保存或 Apply。
- Gemini/Antigravity 流式工具标记会转换成结构化调用，工具命令失败后保留调用配对并继续生成。
- 火山方舟 Coding Plan 的 429 改为有上限的延迟退避，尊重短 `Retry-After`，并降低单账号并发以避免重试风暴。
- Grok 4.6 继续保留视觉能力；仅在上游路由拒绝图片时自动降级为文本续跑，不再让整个任务因单个图片项终止。

## 发布文件

- `Codex-Router-Installer-1.7.6-windows-x64.exe`
