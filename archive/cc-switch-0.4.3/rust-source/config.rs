...
pub struct DeployConfig {
    ...
    #[serde(default = "default_cc_switch_profile_name")]
    pub cc_switch_profile_name: String,
    #[serde(default)]
    pub cc_switch_profile_id: String,
    #[serde(default = "default_true")]
...
fn default_cc_switch_profile_name() -> String {
...
impl Default for DeployConfig {
    fn default() -> Self {
        Self {
            ...
            cc_switch_profile_name: default_cc_switch_profile_name(),
            cc_switch_profile_id: String::new(),
    ...
    pub fn load(path: &std::path::Path) -> anyhow::Result<Self> {
        ...
        for model in &mut cfg.models {
            if model.model.eq_ignore_ascii_case("gpt-5.6") {
                ...
                {
                    model.alias = "GPT-5.6 Sol".to_owned();
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn legacy_config_gets_a_safe_cc_switch_profile_name() {
        let config: RouterConfig = serde_json::from_str(r#"{"deploy":{}}"#).unwrap();
        assert_eq!(config.deploy.cc_switch_profile_name, "Codex-Router");
        assert!(config.deploy.cc_switch_profile_id.is_empty());
    ...
    fn legacy_bare_gpt_56_is_migrated_to_sol_on_load() {
        ...
        assert_eq!(config.models[0].alias, "GPT-5.6 Sol");
